...
class ResPartner(models.Model):
    ...
    contact_type = fields.Selection([('call', 'Mobile'), ('mail', 'Email'), ('both', 'Both')], string='Contact Type',
                                    default='call', required=True)
    ...
    @api.model
    def _get_computed_name(self, lastname, middlename, firstname):
        """Compute the 'name' field according to splitted data.
        You can override this method to change the order of lastname and
        firstname the computed name"""
        order = self._get_names_order()
        if order == 'last_first_comma':
            return ", ".join((p for p in (lastname, middlename, firstname) if p))
    ...
    def _check_name(self):
        ...
        for record in self:
            if all((
                ...
                not (record.firstname or record.middlename or record.lastname)
            )):
    ...
    def _check_value(self):
        for record in self:
            if record.mobile is None and (record.contact_type == 'call' or record.contact_type == 'both'):
                raise ValidationError(_('mobile Number  is required'))
            if record.email is None and (record.contact_type == 'mail' or record.contact_type == 'both'):
                raise ValidationError(_('emaill address  is required'))
